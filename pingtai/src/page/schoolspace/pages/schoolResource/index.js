...
import { Input, Col, Divider, Pagination, Tag, message } from 'antd';
...
export default connect(mapStateToProps, mapDispatchToProps)(class Vr extends React.Component {
    state = {
        "vrList": [],
        "tagList": [],
        "sonTagList": [],
        ...
        "resourceName": "",
        "total": 0,
        "classifyIndex": sessionStorage.classifyIndex ? [JSON.parse(sessionStorage.classifyIndex)] : [],
        'sonIndex': [],
        "requestFlag": true,
        "sonFlag": true,
        "oftenFlag": true
    ...
    //获取列表数据
    getVrlist = () => {
        let { classifyIndex, sonIndex, pageNum, pageSize, resourceName } = this.state;
        console.log(classifyIndex)
        getVrlist({
            classifyIds: classifyIndex.concat(sonIndex).filter(v => !v.notSend).map(v => v.id),
            pageSize,
            pageNum,
            resourceName
        }).then(res => {
    ...
    changeS = (v) => {
        ...
        this.setState({ sonIndex: res, classifyIndex }, () => {
            this.getVrlist();
        })
    }
    //切换显示多少条
    ...
    reset = () => {
        sessionStorage.removeItem('classifyIndex');
        this.setState({ classifyIndex: [], sonIndex: [] },()=>this.getVrlist());
    }
    render() {
        let { total, resourceName, pageNum, pageSize, vrList, tagList, classifyIndex, sonIndex, sonTagList } = this.state;
        return (
            <>
                ...
                <div className='vr_box'>
                    <div className="top_wai">
                        <div className="top">
                            <div className="filter">
                                <div className="left">
                                    ...
                                    <p>
                                        ...
                                        {
                                            classifyIndex.map((v, i) => <span key={i} style={{ marginRight: '10px' }}>{v.classifyName}</span>)
                                        }

                                        {
                                            sonIndex.map((v, i) => <span key={i} style={{ marginRight: '10px' }}>{v.classifyName}</span>)
                                        }
                                    </p>
                                    <i onClick={this.reset}>重置</i>
                                </div>
                            </div>
                            <div className="classify">
                                <div className="hot">
                                    ...
                                    <div className="right">
                                        <div className="right_list">
                                            <CheckableTag
                                                className={setName('item', { 'active': !classifyIndex.length ? true : false })}
                                                onChange={this.allFn}
                                            >
                                                全部
                                                </CheckableTag>
                                            {
                                                tagList.map((v, i) => {
                                                    return <CheckableTag
                                                        className={setName('item', { 'active': classifyIndex.filter(item => item.id === v.id).length ? true : false })}
                                                        key={v.id}
                                ...
                                {
                                    sonTagList.length ? <div className="type" style={{ display: classifyIndex.length > 1 ? 'none' : 'flex' }}>
                                        <div className="left">
                                            <span>子类</span>
                                            <i>|</i>
                                        </div>
                                            ...
                                            <div className="right_list">
                                                {
                                                    sonTagList.map((v, i) => {
                                                        return <CheckableTag
                                                            key={v.id}
                                                            className={setName('item', { 'active': sonIndex.filter(item => item.id === v.id).length ? true : false })}
                                                            onChange={() => { this.changeS(v, i) }}
                                                        >
                                                            {v.classifyName}
                                                        </CheckableTag>
                                                    })
                    ...
                    <div className="bottom_wai">
                        <div className="bottom">
                            {
                                vrList && vrList.length ? vrList.map((v, i) => {
                                    return <Col key={v.id} className='item' span={6}>
                                        <div className="top">
                                            <div className="left">
                                                <div className="tag">子类</div>
                                                <img src={getShowImage(v.coverFileId)} alt="" />
                                            </div>
                                            <div className="right">
                                                <h2>
                                                    <span>{v.resourceName}</span>
                                                    {
                                                        v.ifAdapter === 1 ? <span>适配软硬件：zSpace</span> : null
                                                    }
                                                </h2>
                                                <p>
                                                    {v.content}
